...
grid = [['.' for j in range(0, 500+100+max_y+2)] for i in range(0, max_y + 3)]
...
def draw_line_in_grid(grid, tuple_1, tuple_2):
    if tuple_1[0] != tuple_2[0]:
        for i in range(min(tuple_1[0], tuple_2[0]), max(tuple_1[0], tuple_2[0]) + 1):
            grid[tuple_1[1]][i+1] = '#'
    else:
        for i in range(min(tuple_1[1], tuple_2[1]), max(tuple_1[1], tuple_2[1])):
            grid[i][tuple_1[0] + 1] = '#'
